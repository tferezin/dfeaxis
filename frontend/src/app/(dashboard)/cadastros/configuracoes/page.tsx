...
import { Save, Clock, Globe, Bell, AlertTriangle, Eye, FileCheck, Mail } from "lucide-react"
...
export default function ConfiguracoesPage() {
  ...
  // Soft launch: a ativação de Produção é manual via suporte enquanto a
  // integração SEFAZ não estabiliza. O modal vira um CTA pra mandar e-mail
  // pra suporte@dfeaxis.com.br ao invés do antigo fluxo senha+checkbox.
  // Quando `PROD_ACCESS_ALLOWED=true` no Railway, voltar pro fluxo direto.
  ...
  const handleAmbienteChange = (next: string) => {
    ...
    // Mudança pra Homologação é caminho seguro — persiste direto
    updateSettings({ sefazAmbiente: "2" })
    apiFetch("/tenants/settings", {
      method: "PATCH",
      body: JSON.stringify({ sefaz_ambiente: "2" }),
    }).catch((e) => {
    ...
    <div className="space-y-4">
      ...
      <Dialog open={prodModalOpen} onOpenChange={setProdModalOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Solicitar liberação de Produção</DialogTitle>
            <DialogDescription>
              Durante a fase inicial, a ativação é feita manualmente pela nossa equipe.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3">
              ...
              <div className="text-xs text-amber-900 space-y-1">
                ...
                <ol className="list-decimal pl-4 space-y-0.5">
                  <li>Envie um e-mail para <strong>suporte@dfeaxis.com.br</strong> solicitando liberação</li>
                  <li>Nossa equipe revisa o setup do seu CNPJ e certificado A1</li>
                  <li>Após habilitação, você consegue alternar para Produção pelo painel</li>
                </ol>
              </div>
            </div>

            <div className="rounded-lg border border-blue-200 bg-blue-50 p-3 text-xs text-blue-900">
              Etapa temporária enquanto estabilizamos a integração com a SEFAZ — em breve a liberação será automática.
            </div>
          </div>
